...
export default function MyOrders() {
  ...
  // সার্ভার থেকে অর্ডার ফেচ করা
  useEffect(() => {
    if (session?.user) {
      fetch(`${process.env.NEXT_PUBLIC_SERVER_URL}/orders/my-orders`, {
        method: "GET",
  ...
  const handleCancel = async (orderId) => {
    ...
    try {
      ...
      if (res.ok) {
        toast.success("অর্ডারটি বাতিল হয়েছে।");
        // লিস্ট থেকে বাতিল হওয়া অর্ডারটি সরিয়ে নেওয়া
        ...
        <table className="w-full text-left border-collapse min-w-[600px]">
          ...
          <tbody className="text-sm text-[#1b2559] divide-y divide-slate-50">
            {orders.length > 0 ? (
              orders.map((order) => (
                <tr key={order._id}>
                  <td className="py-4 px-2 font-mono text-xs text-slate-400">{order._id.slice(-6)}</td>
                  <td className="py-4 px-2 font-bold">{order.productName || "Product Name"}</td>
                  ...
                  <td className="py-4 px-2 text-right">
                    {order.status === "Pending" && (
                      <button 
                        ...
                        className="bg-red-50 hover:bg-red-100 text-red-600 px-3 py-1.5 rounded-xl text-xs font-bold transition-colors"
                      >
                        Cancel Order
                      </button>
                    )}
              ...
              <tr>
                <td colSpan="4" className="py-10 text-center text-slate-400">কোনো অর্ডার পাওয়া যায়নি।</td>
              </tr>
            )}
          </tbody>
        </table>
